...
def search(request, field=None, value=None, order=None):
    ...
    if field=='keyword' or (not field and value):
        ...
        q = (Q(marginalia__icontains=value) |
        ...
        display_field='Unverified'; display_value='All'
        result_list = qs.filter(verification='U')
    elif field=='ghosts':
        display_field='Ghosts'; display_value='All'
    ...
    if order=='date':
        result_list = sorted(result_list,
                             key=lambda c: (int(c.issue.start_date or 0),
                                            strip_article(c.issue.edition.title).lower(),
                                            strip_article(c.location.name_of_library_collection or '')))
    elif order=='title':
        result_list = sorted(result_list,
                             key=lambda c: (strip_article(c.issue.edition.title).lower(),
                                            int(c.issue.start_date or 0)))
    elif order=='location':
        result_list = sorted(result_list,
                             key=lambda c: (strip_article(c.location.name_of_library_collection or ''),
                                            int(c.issue.start_date or 0)))
    elif order=='stc':
        result_list = sorted(result_list,
                             key=lambda c: (c.issue.year or '',
                                            strip_article(c.location.name_of_library_collection or '')))
    elif order.upper()=='WC':
